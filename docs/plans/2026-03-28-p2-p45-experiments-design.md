...
| 5 | Surrogates | Exp B (DT) + Exp C (RuleFit) với SHAP feature selection |
...
├── models/
│   ├── rulefit_surrogate.py     # RuleFit via imodels.RuleFitClassifier
│   └── rule_extractor.py        # DT/RuleFit → human-readable IF-THEN text
└── evaluation/
...
### 5.2 Exp B — Decision Tree Ablation (`p3a_exp_b_dt.py`)

Fix `max_depth=5`. Không dùng `class_weight='balanced'` — window imbalance đã được xử lý ở data level bằng `max_windows_per_seq`; thêm class_weight khi attack là majority class sẽ up-weight nhầm class và gây FN cao. Ablation matrix (evaluate trên **test set**):
...
**Pareto frontier plot:** depth ∈ {3, 5, 7, 10} × Attack Fidelity vs #Rules → figure cho paper.

### 5.3 Exp C — RuleFit Ablation (`p3b_exp_c_rulefit.py`)

Cùng ablation matrix như Exp B, dùng `imodels.RuleFitClassifier`. Thêm metric:
- **Rule complexity** = tổng literals trên tất cả rules (lower = more interpretable)

So sánh DT vs RuleFit side-by-side trong cùng table → trả lời RQ1.
...
Fix config = DT-Soft-T*-Full (winner từ Exp B). Cùng Teacher.
...
| Evaluation §1: Ablation | C5 (Exp B+C) | Table: distilled vs direct, DT vs RuleFit |
...
- SHAP details, full RuleFit vs DT comparison
...
| P3 | Tháng 5 week 2 | Exp B + C + Phase ablation | Ablation tables |
| P4 | Tháng 6 week 1 | C7+C8+C9+C10 | Core results complete |
| P4.5 | Tháng 6 week 3 | Paper draft | Submit to ACSAC |

**Critical path:** P2 checkpoint → Exp B (winner config) → Phase ablation (dùng winner config) → C7 rules → C9 eBPF.
...
Thêm vào `guepard-shield-model/pyproject.toml`: `imodels`, `shap`.
