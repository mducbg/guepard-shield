...
với nhân hệ điều hành qua lời gọi hệ thống (\gls{syscall}), chuỗi \gls{syscall}
là nguồn quan sát quan
trọng cho các \gls{hids}. Tuy
...
Luận văn đề xuất Guepard Shield, một \gls{hids} lai lấp khoảng
trống đó bằng cách tách
giai đoạn học biểu diễn ngoại tuyến ra khỏi giai đoạn thực thi thời
gian thực. Hướng tiếp
cận là chưng cất tri thức từ Transformer Teacher sang một \gls{dfa}
Student đủ nhỏ để nạp vào \gls{bpf} map và truy vấn với độ phức tạp $O(1)$
mỗi \gls{syscall}. \Gls{dfa} vừa
phù hợp với ràng buộc của \gls{ebpf} Verifier, vừa bảo toàn tri thức ngữ
cảnh mà Teacher đã học.

Pipeline gồm bốn giai đoạn: token hóa tên \gls{syscall} và tạo cửa sổ trượt
(Giai đoạn~1),
huấn luyện Decoder-only Transformer trên dữ liệu bình thường (Giai
đoạn~2), rời rạc hóa
vector trạng thái ẩn bằng K-Means để xây dựng bảng chuyển \gls{dfa} (Giai
đoạn~3), và nạp bảng
chuyển vào \gls{bpf} map để thực thi qua \gls{ebpf} (Giai đoạn~4).

Thực nghiệm trên \gls{lid-ds}-2021 cho thấy Teacher đạt \gls{auroc} = 0.8503 và
\gls{pr-auc} = 0.7093 trên
gần 100 triệu window kiểm thử không dùng nhãn tấn công khi huấn
luyện. \Gls{dfa} Student tại
...
Teacher, xác nhận
tính khả thi của hướng chưng cất Transformer sang \gls{dfa} cho bài toán
giám sát \gls{syscall}
thời gian thực.
\begin{flushright}
  \begin{tabular}{@{}c@{}}
    Học viên\\
