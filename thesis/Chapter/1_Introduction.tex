...
(perimenter-based security) như tường lửa hay IDS mạng dần bộc lộ
...
\begin{enumerate}
  \item Các phương pháp phát hiện dựa trên học sâu (Deep Learning)
    như Transformer, LSTM \cite{hochreiter1997lstm} hoặc GRU
...
RNN của Weiss và các cộng sự \cite{weiss2018extracting}. Các công trình
...
toán syscall, kiến trúc Transformer và môi trường eBPF\@.
...
trình huấn luyện và trích xuất DFA được thực hiện trên các bộ dữ liệu
chuẩn về syscall như LID-DS \cite{grimmer2022dataset} và bộ dữ liệu do
chúng tôi tự thu thập trên các hệ thống thực tế (DongTing). Việc sử
dụng đa dạng các bộ dữ liệu giúp đánh giá được khả năng tổng quát hóa
...
khai thác lỗ hổng CVE phức tạp.
...
đưa DFA tới trạng thái không xác định hoặc trạng thái tấn công
...
Thứ nhất, nghiên cứu góp phần giải quyết bài toán hiệu năng của các hệ
thống HIDS hiện đại. Bằng cách chưng cất tri thức từ các mô hình nơ-ron
...
mỗi mili giây độ trễ thêm vào đường đi syscall có thể dẫn tới thiệt hại
đáng kể về trải nghiệm người dùng và chi phí tài nguyên.

Thứ hai, việc sử dụng eBPF làm công cụ thực thi chính sách giúp tăng
...
nguồn không ổn định), eBPF cung cấp một môi trường thực thi được kiểm
...
khe về tài nguyên. Quy trình trích xuất và tối ưu DFA từ Transformer có
...
cảnh báo bất thường với các kỹ thuật tấn công trong ma trận MITRE
ATT\&CK \cite{strom2018mitre}. Điều này giúp thu hẹp khoảng cách giữa việc "phát hiện một cái
