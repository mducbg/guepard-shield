...
cơ chế giám sát thực thi thời gian thực bằng \gls{ebpf}. Bên cạnh đó,
chương này cũng hệ thống hóa các kiến thức nền tảng về mạng
Transformer Decoder-only, lý thuyết \gls{dfa},
và kiến trúc \gls{ebpf} trong nhân Linux, làm cơ sở khoa học để giải quyết
...
bất thường dựa trên \gls{syscall} cần được hình thức hóa dưới góc độ của
...
trong đó mỗi $x_t \in \mathcal{S}$ là \gls{syscall} tại thời điểm $t$.
...
Deterministic Finite Automaton (\gls{dfa}), ký hiệu là $M = (Q, \Sigma,
\delta, q_0, F)$.
Trong đó:
\begin{itemize}
  \item $Q$ là tập hữu hạn các trạng thái, được trích xuất từ việc
    phân cụm không gian vector ẩn của Transformer.
  \item $\Sigma \subseteq \mathcal{S}$ là bảng chữ cái (tập \gls{syscall}).
...
ghi nhớ ngữ cảnh dài thông qua cơ chế chú ý, trong khi \gls{dfa} truyền thống
...
duy trì tính "Markov" cần thiết cho \gls{dfa} mà không làm bùng nổ số lượng
trạng thái.

Một vấn đề nghiên cứu then chốt khác là khả năng chống lại các cuộc
tấn công bắt chước (Mimicry Attacks). Trong bối cảnh \gls{syscall}, kẻ tấn
công có thể chèn thêm các lời gọi hệ thống "vô hại" (no-op hoặc các
\gls{syscall} bình thường) vào giữa các bước của mã độc để làm thay đổi
biểu diễn vector của chuỗi, từ đó đánh lừa các hệ thống phát hiện
bất thường. Việc trích xuất \gls{dfa} cần đảm bảo rằng cấu trúc whitelist
...
người dùng. Khả năng chịu đựng của \gls{dfa} đối với các biến đổi trình tự
...
(\gls{syscall}-level anomaly detection) trên các máy chủ Linux chạy các
...
thay đổi phân phối chuỗi \gls{syscall} để đánh lừa mô hình học máy. Các
rootkit mức kernel can thiệp trực tiếp vào cấu trúc dữ liệu của hệ
điều hành hoặc các hệ thống phát hiện xâm nhập mức mạng (Network \gls{ids})
...
Các nghiên cứu về \gls{hids} dựa
trên \gls{syscall} đã trải qua nhiều giai đoạn phát triển, từ các thuật
toán thống kê đơn giản đến các mô hình học sâu phức tạp.

Phương pháp kinh điển đầu tiên là \gls{stide} (Sequence Time-Delay
Embedding) do Forrest và các cộng sự đề xuất \cite{forrest1996sense}.
\Gls{stide} hoạt động dựa trên cơ chế n-gram, xây dựng một cơ sở dữ liệu
...
chuỗi \gls{syscall} con nào có độ dài $n$ xuất hiện mà không nằm trong cơ
sở dữ liệu bình thường sẽ bị coi là bất thường. Mặc dù \gls{stide} có tốc
...
(vocabulary) của các \gls{syscall} mở rộng.

Sự trỗi dậy của học sâu đã thay đổi căn bản cách tiếp cận này.
DeepLog \cite{du2017deeplog} giới thiệu việc áp dụng \gls{rnn} \cite{hochreiter1997lstm} cho bài toán phát hiện bất thường thông qua việc dự
đoán phần tử tiếp theo (next-token prediction) trên chuỗi log hoặc
chuỗi \gls{syscall}. Khi xác suất dự đoán của \gls{syscall} tiếp theo thực tế
...
\cite{mikolov2013word2vec} truyền thống trên chuỗi \gls{syscall}, Mvula và các cộng sự
\cite{mvula2023evaluating} khuyến nghị sử dụng các phương pháp mã hóa
nén như \gls{bpe} \cite{sennrich2016bpe}. Dù đạt độ chính xác cao, các mô
hình dựa trên \gls{lstm} gặp khó khăn lớn trong việc xử lý song song khi
...
ứng dụng để mô hình hóa chuỗi \gls{syscall}. Nghiên cứu của Fournier và các
cộng sự \cite{fournier2023language} đã chỉ ra rằng các mô hình ngôn
ngữ lớn (như Transformer và Longformer) đạt độ chính xác \gls{f1} và \gls{auroc}
vượt trội (>95\%) trên các bài toán phát hiện bất thường nhờ khả năng
nắm bắt tốt các quan hệ ngữ cảnh dài hạn giữa các \gls{syscall} đứng xa
nhau. TransCall \cite{alsiam2026transcall} cũng khai thác Transformer
để phát hiện mã độc zero-day dưới dạng xử lý chuỗi \gls{syscall} tương tự
...
cầu độ trễ xử lý của mỗi \gls{syscall} phải ở mức micro giây. Khoảng cách
...
như \gls{lime} \cite{ribeiro2016lime} hay \gls{shap} \cite{lundberg2017shap} vốn tốn thời gian suy luận phụ và không đảm bảo
...
xác nhưng nặng nề, trong khi Student (như \gls{dfa}) cần đạt được độ trung
...
vốn không duy trì một trạng thái ẩn tuần tự duy nhất như \gls{rnn}. Tuy
...
thể xấp xỉ trung thực hành vi của Teacher. Việc trích xuất \gls{dfa} từ Transformer yêu
...
Việc chuyển đổi tri thức sang \gls{dfa} mang lại lợi ích kép cho an ninh hệ
thống. Về mặt vận hành, \gls{dfa} cho phép triển khai cơ chế whitelist động
có khả năng phản hồi ở tốc độ của phần cứng hoặc kernel. Về mặt phân
tích, cấu trúc đồ thị của \gls{dfa} cho phép các chuyên gia bảo mật truy
vết ngược lại các lộ trình tấn công dưới dạng các chuỗi trạng thái
tường minh. Nếu một chuỗi \gls{syscall} bị gắn attack, \gls{dfa} có thể chỉ ra
chính xác \gls{syscall} nào và tại ngữ cảnh nào đã gây ra sự vi phạm, điều
...
như \gls{ebpf}. Hầu hết các công trình hiện nay mới chỉ tập trung vào việc
đạt được độ trung thực cao về mặt toán học mà chưa xem xét đến giới
hạn bộ nhớ của \gls{bpf} Maps hoặc khả năng xử lý các chuỗi \gls{syscall} có
...
chuyển \gls{dfa} vào \gls{ebpf}.

\subsection{Thực thi chính sách bảo mật thời gian thực qua eBPF}
Công nghệ \gls{ebpf} (Extended Berkeley Packet Filter) đại diện cho một
...
(hooks) như tracepoints hoặc \gls{lsm} (Linux Security Modules) với chi phí
chuyển đổi ngữ cảnh (context switch) bằng không.

Các công cụ bảo mật cloud-native phổ biến hiện nay như Falco và
Tetragon \cite{her2025indepth} tận dụng \gls{ebpf} để thu thập telemetry và
...
escapes) chỉ trong 1.178 giây với mức chiếm dụng \gls{cpu} tối ưu (~73\%)
...
học máy vào thực thi trong kernel qua \gls{ebpf}. Zhang và các cộng sự
\cite{zhang2024realtime} đã thiết kế và triển khai một mạng nơ-ron
nhân tạo trực tiếp trong kernel bằng \gls{ebpf} bằng cách lượng tử hóa mô
hình sang số học số nguyên (integer-only arithmetic) để vượt qua giới
hạn stack 512 bytes của \gls{ebpf} verifier. Mô hình đạt thời gian suy luận
cực nhanh (3-5 micro giây) nhưng chương trình \gls{ebpf} này cực kỳ phức
...
(decision tree) trong \gls{ebpf} tăng 20\% hiệu suất so với không gian người dùng.
...
\begin{table}[htbp]
  ...
  \resizebox{\textwidth}{!}{
    \begin{tabular}{|l|p{3.5cm}|p{3.5cm}|p{2cm}|p{2cm}|}
      ...
      \textbf{\gls{stide} / N-gram} \cite{forrest1996sense} & Tốc độ nhanh,
      thuật toán đơn giản, dễ cài đặt. & Bỏ lỡ các ngữ cảnh dài hạn,
      độ chính xác thấp trên hệ thống phức tạp. & Có (Thống kê tần
      suất) & Có (Thông qua cấu trúc seccomp-\gls{bpf} thô) \\ \hline
      \textbf{\gls{lstm} / DeepLog} \cite{du2017deeplog} & Nắm bắt ngữ cảnh
...
sách \gls{ebpf}. Chưa có giải pháp nào tự động chưng cất tri thức từ mô
hình Transformer thành một bảng chuyển trạng thái \gls{dfa} tối ưu, sau đó
nạp bảng chuyển này vào các cấu trúc \gls{bpf} Maps để thực thi giám sát
...
của Transformer với hiệu năng thực thi của \gls{ebpf}.
...
một từ vựng hữu hạn $\Sigma$ đại diện cho tập hợp các \gls{syscall} được
...
tiếp theo (next-token prediction) trên tập dữ liệu chuỗi \gls{syscall} lành tính:
\begin{equation}
  \label{eq:loss}
...
thông tin lịch sử của luồng \gls{syscall} từ bước $1$ đến $t$. Đặc tính này
...
Một Automata hữu hạn đơn định (Deterministic Finite Automaton - \gls{dfa})
...
đơn định. Về mặt hình thức, một \gls{dfa} được định nghĩa là một bộ 5 thành phần:
\begin{equation}
...
\begin{itemize}
  ...
  \item $\Sigma$ là bảng chữ cái đầu vào hữu hạn (trong bài toán
      \gls{syscall} là tập hợp các chỉ số hoặc tên của lời gọi hệ thống nằm
    trong Vocabulary).
...
Trong bài toán phát hiện bất thường sử dụng \gls{dfa}, tập các trạng thái
chấp nhận $F$ thường được thay thế bằng tập các trạng thái tấn công
(attack hoặc abnormal states) $A \subseteq Q$. Khi chuỗi \gls{syscall} dẫn
\gls{dfa} đến một trạng thái thuộc $A$, hệ thống sẽ phát tín hiệu cảnh báo xâm nhập.

Đặc tính quan trọng nhất khiến \gls{dfa} trở thành mục tiêu triển khai lý
...
\begin{equation}
  ...
  |Q| \times |\Sigma|
\end{equation}
\gls{dfa} không yêu cầu bất kỳ phép tính số thực dấu phẩy động nào và có
...
\gls{ebpf} là một công nghệ mang tính cách mạng trong nhân Linux, cho phép
...
Nguyên thủy, \gls{bpf} (Classic \gls{bpf} - cBPF) được giới thiệu vào năm 1992
\cite{mccanne1993bpf} như một cơ chế lọc gói tin hiệu quả trong nhân. Tuy nhiên, kể từ
phiên bản nhân Linux 3.15, \gls{ebpf} đã mở rộng phạm vi ứng dụng từ mạng
sang giám sát toàn diện hệ thống. Sự khác biệt cốt lõi nằm ở việc
\gls{ebpf} sở hữu tập lệnh rộng hơn (64-bit thay vì 32-bit), số lượng thanh
ghi tăng từ 2 lên 10, và khả năng gọi các hàm helper trong nhân. Đặc
biệt, \gls{ebpf} giới thiệu cơ chế JIT (Just-In-Time) compiler, cho phép
biên dịch mã bytecode \gls{ebpf} trực tiếp sang mã máy bản địa của \gls{cpu},
...
Các chương trình \gls{ebpf} hoạt động theo cơ chế hướng sự kiện
...
\gls{syscall}), kprobes/kretprobes (móc vào các hàm nhân bất kỳ), hoặc các
\gls{lsm} hooks (Linux Security Modules) như AppArmor hay SELinux. Khi sự
kiện xảy ra, chương trình \gls{ebpf} được thực thi để phân tích ngữ cảnh sự
kiện đó. Khả năng truy cập vào cấu trúc dữ liệu của nhân thông qua
các điểm móc này cho phép \gls{ebpf} quan sát được các thông tin nhạy cảm
như PID, UID, tham số \gls{syscall}, và ngữ cảnh tiến trình cha-con.

\subsection{Trình xác thực eBPF (Verifier) và Tính an toàn}
Để đảm bảo an toàn tuyệt đối cho hệ điều hành, nhân Linux tích hợp
một trình xác thực tĩnh (verifier) thực hiện kiểm tra mã nguồn \gls{ebpf}
...
\begin{itemize}
  \item \textbf{Giới hạn ngăn xếp (Stack limit):} Kích thước stack
    tối đa của một chương trình \gls{ebpf} chỉ là 512 bytes. Mọi biến cục
...
Vì kích thước stack bị giới hạn ở 512 bytes, các chương trình \gls{ebpf}
không thể lưu trữ các cấu trúc dữ liệu lớn trực tiếp trên stack. Thay
vào đó, \gls{ebpf} cung cấp cấu trúc \gls{bpf} Maps. Đây là các kho lưu trữ dữ
liệu dạng khóa - giá trị (key-value stores) hiệu năng cao nằm trong
bộ nhớ nhân, hỗ trợ tra cứu với độ phức tạp $O(1)$. Các loại \gls{bpf} Maps
phổ biến bao gồm: \texttt{BPF\_MAP\_TYPE\_HASH},
\texttt{BPF\_MAP\_TYPE\_ARRAY}, hoặc \texttt{BPF\_MAP\_TYPE\_LRU\_HASH}.

Bảng chuyển trạng thái của \gls{dfa} $\delta(q, s) = q'$ có thể được biểu
diễn trong kernel bằng một Hash Map của \gls{bpf}. Trong cấu trúc này, khóa
(key) là một cặp số nguyên bao gồm định danh trạng thái hiện tại và
mã số lời gọi hệ thống (\gls{syscall} ID), còn giá trị (value) tương ứng là
trạng thái kế tiếp. Khi một \gls{syscall} được kích hoạt, chương trình \gls{ebpf}
thực hiện phép tra cứu $O(1)$ trên Map này để cập nhật trạng thái \gls{dfa}
...
Thách thức lớn nhất của \gls{ebpf} truyền thống là sự phụ thuộc vào cấu
trúc dữ liệu cụ thể của từng phiên bản nhân Linux (kernel version
dependency). Để giải quyết vấn đề này, công nghệ BTF (\gls{bpf} Type
...
dùng thực hiện việc điều chỉnh địa chỉ (relocation) mã bytecode \gls{ebpf}
tại thời điểm chạy. Nhờ đó, một chương trình \gls{ebpf} có thể chạy trên
...
Một đặc tính ưu việt của \gls{bpf} Maps là hỗ trợ các thao tác cập nhật
nguyên tử (atomic map updates). Điều này cho phép một tiến trình chạy
ở không gian người dùng có thể ghi đè bảng chuyển trạng thái \gls{dfa} mới
vào \gls{bpf} Maps mà không cần dừng hoặc dỡ bỏ chương trình \gls{ebpf} đang giám
sát trong kernel. Khả năng này đảm bảo hệ thống có thể cập nhật động
các chính sách bảo mật hoặc điều chỉnh trạng thái \gls{dfa} khi phát hiện
...
Tóm lại, công nghệ \gls{ebpf} không chỉ cung cấp một hạ tầng thu thập dữ
...
của \gls{bpf} Maps là những yếu tố quyết định đến cách chúng ta thiết kế và
rời rạc hóa mô hình \gls{dfa} từ Transformer. Việc tận dụng BTF và CO-RE
