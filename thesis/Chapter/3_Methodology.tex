...
2), chưng cất tri
thức sang biểu diễn \gls{dfa} Student thông qua gom cụm K-Means trên các
vector trạng thái ẩn
(Giai đoạn 3), và thực thi bảng chuyển trạng thái tại runtime bằng
\gls{ebpf} (Giai đoạn 4).
...
Ở giai đoạn ngoại tuyến, dữ liệu \gls{syscall} bình thường được tiền xử lý
và token hóa (Giai
...
các vector trạng
thái ẩn lớp cuối của nó được trích xuất và rời rạc hóa thành một \gls{dfa}
compact thông qua
gom cụm K-Means và xây dựng bảng chuyển trạng thái (Giai đoạn 3). Ở
giai đoạn trực
tuyến, bảng chuyển trạng thái \gls{dfa} được nạp vào \gls{bpf} Maps
trong kernel;
chương trình \gls{ebpf}
sau đó kiểm tra mỗi \gls{syscall} với chi phí $O(1)$ trên mỗi sự kiện
(Giai đoạn 4).

Cơ chế tương tác động giữa kernel space và user space được mô tả trong
Hình~\ref{fig:runtime}. Khi \gls{dfa} phát hiện một bước chuyển trạng thái
trong vùng nghi vấn
(Suspect), sự kiện đó được chuyển lên Rust Agent ở user space thông
qua \gls{bpf} perf ring buffer
để đánh giá sâu hơn bởi mô hình Transformer; nếu được xác định là
false positive, \gls{dfa}
...
Một điểm thiết kế quan trọng là hệ thống không ghi lại toàn
bộ chuỗi \gls{syscall}
tại runtime. Thay vào đó, chỉ tồn tại các cửa sổ trượt có kích thước
$W$ được duy trì liên tục theo từng định danh luồng (\gls{tid}). Mỗi
\gls{syscall} mới đẩy cửa sổ
tiến một bước, và \gls{dfa} cập nhật trạng thái hiện tại dựa trên
token tương ứng.

\section{Giai đoạn 1 - Tiền xử lý dữ liệu}
\label{sec:phase1}

Giai đoạn này chuyển đổi các bản ghi \gls{syscall} thô từ bộ dữ liệu
\gls{lid-ds}-2021
...
Mỗi sự kiện \gls{syscall} được ánh xạ từ tên lời gọi hệ thống sang một chỉ
số nguyên thông
...
$f_{\text{min}}$ để loại bỏ các \gls{syscall} xuất hiện
quá thưa thớt trong dữ liệu huấn luyện \cite{mvula2023evaluating}.
Các tên \gls{syscall} không
được nhận diện tại thời điểm inference được ánh xạ tới token đặc biệt
\texttt{<UNK>}.
Kích thước bộ từ vựng $|\Sigma|$ được xác định để bao phủ hầu hết các lời
gọi hệ thống xuất hiện trong các khối lượng công việc máy chủ bình thường.

Lý do chọn cách token hóa thuần túy dựa trên tên \gls{syscall} là vì tên
\gls{syscall} tạo thành một
bảng chữ cái hữu hạn và ổn định, thỏa mãn trực tiếp yêu cầu của \gls{dfa}
về một bảng chữ cái
đầu vào hữu hạn $\Sigma$ mà không cần kỹ thuật mã hóa phức tạp. Đây
cũng là điều kiện
tiên quyết để chương trình \gls{ebpf} có thể thực hiện ánh xạ từ tên
\gls{syscall} sang token ID bằng
một thao tác tra cứu \gls{bpf} Map đơn giản tại runtime.

Một hướng phát triển được dự kiến là token hóa tổng hợp (composite
tokenization), trong
đó mỗi \gls{syscall} được ánh xạ sang một token hỗn hợp kết hợp tên và nhóm
mã trả về, ví dụ
...
từ vựng và kích
thước \gls{bpf} Map tương ứng, nên được hoãn lại sau khi hệ thống cơ
sở được xác nhận.
...
hình, đảm bảo sự
nhất quán giữa giai đoạn ngoại tuyến và giai đoạn thực thi \gls{ebpf}.
...
\begin{table}[H]
  ...
  \begin{tabular}{|l|c||l|c|}
    \hline
    \textbf{Tên \gls{syscall}} & \textbf{Token ID} &
    \textbf{Tên \gls{syscall}} & \textbf{Token ID} \\ \hline
...
Chuỗi \gls{syscall} theo từng luồng tiến trình được phân đoạn thành các cửa
sổ trượt gối đầu
...
\begin{enumerate}
  ...
  \item \textbf{Tính gọn nhẹ của \gls{dfa}:} Số trạng thái \gls{dfa}
    cần thiết phụ thuộc vào
    kích thước không gian trạng thái ẩn; window nhỏ hơn làm giảm sự
    đa dạng cần mô
    hình hóa, giúp \gls{dfa} chiếm ít bộ nhớ kernel hơn.
\end{enumerate}
...
tệp metadata JSON
của \gls{lid-ds}: một window được gán nhãn \textbf{attack} nếu dấu thời
gian của \gls{syscall} cuối
...
Cần lưu ý rằng bộ dữ liệu \gls{lid-ds} gán nhãn attack cho toàn bộ phần
đuôi của bản ghi kể
...
trực tiếp cho chuỗi
token \gls{syscall} \cite{fournier2023language,alsiam2026transcall}. Mặt nạ
nhân quả đảm bảo
tại mỗi vị trí $t$, vector trạng thái ẩn lớp cuối $h_t^{(L)}$ chỉ
được tính từ ngữ
cảnh $(s_1, \ldots, s_t)$. Điều này phản chiếu
đúng tính nhân quả của luồng \gls{syscall} tại runtime: hệ thống không bao
giờ có thể biết
trước \gls{syscall} tương lai.
...
phát từ bản chất
thời gian của bài toán \gls{syscall}. Trong mô hình encoder hai chiều, mỗi
vị trí có thể
...
vector ẩn đó có
thể đã chứa thông tin từ các \gls{syscall} tương lai trong cùng window, tạo
ra một dạng rò
...
như một trạng thái
nhân quả khi trích xuất bảng chuyển \gls{dfa} ở Giai đoạn 3. Mô hình
Decoder-only với causal
mask tránh vấn đề này: vector $h_t$ là biểu diễn của đúng tiền tố
$(s_1,\ldots,s_t)$,
tương tự một trạng thái tóm tắt lịch sử đã quan sát của luồng \gls{syscall}.
...
điểm bất thường
bằng \gls{nll}, vừa tạo cặp chuyển tiếp $(h_t, s_{t+1}, h_{t+1})$ phục vụ
chưng cất automata.
Nếu mô hình chỉ tạo một embedding cấp window, việc ánh xạ embedding
đó thành các trạng
thái trung gian của \gls{dfa} sẽ kém tự nhiên hơn, vì \gls{dfa} cần cập nhật
trạng thái sau mỗi
\gls{syscall} chứ không chỉ đưa ra một nhãn cho toàn bộ cửa sổ. Do đó,
Decoder-only được chọn
không phải vì encoder yếu hơn về mặt biểu diễn, mà vì nó bảo toàn cấu
trúc nhân quả và
cấu trúc chuyển trạng thái mà pipeline \gls{dfa} yêu cầu.

Cơ chế self-attention được dùng vì hành vi hệ thống không chỉ phụ
thuộc vào \gls{syscall}
...
kiểu quan hệ
khác nhau giữa \gls{syscall}: một head có thể tập trung vào các mẫu I/O lặp
lại, một head
khác vào chuỗi tạo tiến trình hoặc ánh xạ bộ nhớ. Số lượng attention
head $h$ được
lựa chọn để cân bằng giữa năng lực biểu diễn và chi phí trích
xuất trạng thái
ẩn ngoại tuyến.

Vì self-attention tự thân không biết thứ tự của token, mô hình cần
một cơ chế mã hóa
vị trí. Thứ tự là thông tin cốt lõi trong \gls{syscall} \gls{ids}: hai cửa sổ có
cùng tập \gls{syscall}
...
khoảng cách và thứ
tự tương đối giữa các \gls{syscall}, trong khi ít phụ thuộc hơn vào việc
một \gls{syscall} cụ thể
...
toàn bộ thông tin
lịch sử từ đầu \cite{he2016resnet}. Đối với chuỗi \gls{syscall}, điều này
đặc biệt quan
...
khó gom cụm thành
các trạng thái \gls{dfa} ổn định. Nói cách khác, residual connection không
chỉ giúp tối ưu hóa
...
trưng bậc cao hơn.
Kích thước ẩn $d_{\text{ff}}$ lớn hơn kích thước embedding
$d$ nhằm tạo không
...
\gls{syscall} khi đọc
vào và khi dự đoán ra \cite{press2017using}. Với bảng chữ cái \gls{syscall}
tương đối nhỏ,
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
      ...
      add/.style={circle, draw, minimum size=0.45cm, inner sep=0pt,
    ...
    \node[blk, fill=yellow!20, below=0.6cm of emb] (rn1)
    {RMSNorm \emph{(pre-norm)}};
    \node[blk, fill=blue!18, below=of rn1] (mha)
    {Causal Multi-Head Self-Attention $+$ RoPE};
    \node[add, below=0.35cm of mha] (add1)
    {$+$};
    \node[blk, fill=yellow!20, below=0.6cm of add1] (rn2)
    {RMSNorm \emph{(pre-norm)}};
    \node[blk, fill=orange!18, below=of rn2] (ffn)
    {FFN: Linear$_1$ $\to$ GELU $\to$ Linear$_2$};
    \node[add, below=0.35cm of ffn] (add2)
    {$+$};

    % Coordinates for residual connections and dashed box fitting
    \path (emb) -- (rn1) coordinate[pos=0.4] (split1);
    \path (add1) -- (rn2) coordinate[pos=0.4] (split2);
    \coordinate (left_ext1) at ([xshift=-4.2cm]split1);
    \coordinate (left_ext2) at ([xshift=-4.2cm]split2);
    \coordinate (right_ext1) at ([xshift=4.2cm]split1);
    \coordinate (right_ext2) at ([xshift=4.2cm]split2);

    % Dashed box around L-repeated block
    \node[draw=black!50, dashed, rounded corners=4pt,
      fit=(rn1)(add2)(left_ext1)(left_ext2)(right_ext1)(right_ext2),
      inner xsep=7pt, inner ysep=5pt,
    label={[font=\small, text=black!55]right:$\times\, L$}] {};
    % === End transformer block ===

    \node[blk, fill=yellow!20, below=0.6cm of add2] (fnorm)
    {Final RMSNorm};
    \node[blk, fill=red!12, below=of fnorm] (proj)
    {Linear Projection \emph{(tied w/ embedding)}};
    \node[blk, fill=gray!10, below=of proj] (output)
    {Output logits: $\mathbf{z}_t \;\in\;\mathbb{R}^{|\Sigma|}$};
    ...
    \draw[arr] (rn1)   -- (mha);
    \draw[arr] (mha)   -- (add1);
    \draw[arr] (split1) -- ++(-4.0,0) |- (add1.west);
    \draw[arr] (add1)  -- (rn2);
    \draw[arr] (rn2)   -- (ffn);
    \draw[arr] (ffn)   -- (add2);
    \draw[arr] (split2) -- ++(-4.0,0) |- (add2.west);
    \draw[arr] (add2)  -- (fnorm);
    \draw[arr] (fnorm) -- (proj);
    \draw[arr] (proj)  -- (output);
  \end{tikzpicture}
  \caption{Sơ đồ kiến trúc mô hình Transformer Teacher (Decoder-only).}
...
phương pháp phát
hiện bất thường không giám sát (unsupervised anomaly detection)
\cite{pang2021deep} đã
được áp dụng rộng rãi
trong các \gls{hids} dựa trên chuỗi sự kiện
...
Quá trình tối ưu hóa sử dụng AdamW \cite{loshchilov2019adamw} với
lịch trình suy giảm học suất
...
xác hỗn hợp (AMP 16-bit mixed precision) \cite{micikevicius2018mixed}
được áp dụng để giảm thời
gian huấn luyện và
bộ nhớ \gls{gpu} cần thiết.
...
\gls{nll} của \textbf{token cuối cùng} $s_W$ khi
đặt trong ngữ cảnh
...
nhãn window của
\gls{lid-ds}: một window bị gán nhãn attack khi dấu thời gian của \gls{syscall}
...
trực tiếp để rời rạc hóa thành trạng thái \gls{dfa}.

Cách tính điểm bất thường dựa trên \gls{nll} của token cuối cũng được
so sánh với hai
phương án thay thế trong giai đoạn thiết kế:

\begin{itemize}
  \item \textbf{\gls{nll} trung bình toàn window:}
    \[
      \overline{\text{\gls{nll}}}(w) = -\frac{1}{W}\sum_{t=1}^{W}
    ...
    các token đầu window thường thuộc vùng hành vi bình thường (xảy
      ra trước thời điểm
    khai thác), chúng đóng góp giá trị \gls{nll} thấp vào trung bình và pha
    loãng tín hiệu
    tấn công tập trung ở cuối window.
  \item \textbf{\gls{nll} tối đa trong window:}
    ...
    không nhất quán
    với quy tắc gán nhãn của \gls{lid-ds}: token có \gls{nll} cao nhất không bảo
    đảm là token cuối
    cùng, dẫn đến sự không khớp giữa điểm số và nhãn window.
\end{itemize}

Một hạn chế quan trọng hơn của toàn bộ hướng tiếp cận ngưỡng \gls{nll} là
\textbf{tính bất
...
không thực tế. Chính
hạn chế này là lý do kỹ thuật trực tiếp dẫn đến thiết kế \gls{dfa} Student
ở Giai đoạn 3:
...
Số liệu đánh giá chính của mô hình Teacher bao gồm \gls{auroc} và \gls{f1} ở cấp
độ cửa sổ so với
nhãn exploit-timestamp. \Gls{auroc} và \gls{f1} ở cấp độ bản ghi
(recording-level) - thông qua
max-pooling điểm số trên từng bản ghi - được báo cáo như một chẩn
đoán bổ sung để phù
hợp với giao thức đánh giá \gls{lid-ds} tiêu chuẩn \cite{grimmer2022dataset}.

\section{Giai đoạn 3 - Chưng cất DFA Student}
\label{sec:phase3}

Giai đoạn này chưng cất tri thức đã học của mô hình Transformer
Teacher thành một \gls{dfa}
...
điều chỉnh cho
kiến trúc Transformer Decoder-only và bài toán chuỗi \gls{syscall}. Quá
...
nhất, đóng vai trò
tương tự trạng thái ẩn của \gls{rnn} \cite{weiss2018extracting}. Đây là thành
phần tự nhiên nhất
để biểu diễn trạng thái \gls{dfa}, vì một trạng thái \gls{dfa} theo
định nghĩa hình thức ở
...
nhân hệ điều hành, chương trình \gls{ebpf} phải giám sát và duyệt trạng
thái \gls{dfa} trên từng lời gọi hệ thống đơn lẻ (tương ứng với bước nhảy
...
\gls{syscall} thay vì chuyển tiếp tuần tự trên từng \gls{syscall} một.
...
\begin{enumerate}
  \item \textbf{Pha 1 (Gom cụm thưa):} Tập huấn luyện được quét hai
    ...
    sổ. Trong pha này, hệ thống sử dụng một bước trượt (stride) cố định và lưu
  ...
  \item \textbf{Pha 2 (Dựng bước chuyển dày):} Sau khi đã học được
    các tâm cụm, toàn bộ chuỗi \gls{syscall} huấn luyện được stream lại với
    ...
    trạng thái \gls{dfa} tương ứng. Các bước chuyển tiếp trạng thái tuần tự
    ...
    nhưng vẫn xây dựng được \gls{dfa} đúng với từng \gls{syscall} đơn lẻ.
\end{enumerate}
Cách tiếp cận này đảm bảo mỗi trạng thái \gls{dfa} đại diện cho một ngữ
...
\begin{itemize}
  \item \textbf{Mean pooling} $\bar{h} = \frac{1}{W}\sum_{t=1}^{W}
    ...
    thời điểm hiện
    tại. Với chuỗi \gls{syscall}, các token giữa window mang biểu diễn của
    các trạng thái
...
luyện được áp dụng
thuật toán gom cụm K-Means \cite{lloyd1982kmeans} với $K$ cụm. Mỗi
tâm cụm $c_k$ định nghĩa
một trạng thái
\gls{dfa} $q_k \in Q$; một vector $h$ được ánh xạ tới trạng thái $q_k$ trong đó:
...
Về mặt thuật toán, hệ thống sử dụng biến thể mini-batch của K-Means
\cite{sculley2010webscale}
...
giá đánh đổi giữa độ mịn của rời rạc hóa và kích thước \gls{dfa}. Vì vậy,
K-Means ở đây không
...
(fidelity)} và
\textbf{tính gọn nhẹ (compactness)} của \gls{dfa}:

\begin{itemize}
  \item $K$ nhỏ: \gls{dfa} có ít trạng thái và \gls{bpf} Map nhỏ, nhưng tỷ lệ
    xung đột không xác
  ...
  \item $K$ lớn: \gls{dfa} tinh tế hơn và tỷ lệ xung đột thấp hơn, nhưng
    kích thước \gls{bpf} Map
...
- về bản chất là
một \gls{nfa} (Non-deterministic Finite Automaton) - vì cùng một cặp $(A,
s)$ có thể ánh xạ
...
\begin{table}[H]
  ...
  \resizebox{\textwidth}{!}{
    \begin{tabular}{|c|l|p{6.0cm}|p{4.5cm}|}
      ...
      construction): mỗi trạng thái \gls{dfa} là một \textit{tập hợp} trạng
      thái \gls{nfa}. &
      Đảm bảo chính xác; có thể gây bùng nổ trạng thái theo cấp số
      nhân. \\ \hline
      \textbf{S2} & Tăng $K$ & Gom cụm tinh hơn giảm mất thông tin
      khi chiếu, thu
      hẹp xung đột không xác định. & \gls{bpf} Map lớn hơn; cần hiệu chuẩn
      lại $K$. \\ \hline
      ...
      \gls{dfa} gọn nhẹ; khai thác phân phối lệch của các bước
      chuyển. \textbf{Ứng
      viên chính.} \\ \hline
    \end{tabular}
  }
...
Số liệu chính để đánh giá \gls{dfa} được chọn là \textbf{tỷ lệ phát hiện
  cấp độ bản ghi
...
DR đặt câu hỏi đúng về mục đích vận hành: \textit{``Với một bản ghi
  chứa tấn công thực
sự, \gls{dfa} có bắt được ít nhất một khoảnh khắc tấn công không?''} Đơn vị
bản ghi có ý
nghĩa vận hành rõ ràng hơn đơn vị window riêng lẻ và loại bỏ được ảnh
hưởng của phần
lớn window được gán nhãn attack trong \gls{lid-ds} mà thực chất chứa hành
vi bình thường xảy
ra sau khi khai thác đã kết thúc (phân tích chi tiết trong Chương 5).

Các số liệu phụ chỉ mang tính chẩn đoán:
\begin{itemize}
  \item \textbf{\gls{tpr}\_window}: Tỷ lệ gắn attack trên nhãn window thô -
    bị pha loãng bởi
    nhiễu nhãn \gls{lid-ds}, không dùng để so sánh chất lượng giữa các cấu hình.
  \item \textbf{Fidelity}: Tỷ lệ đồng thuận giữa quyết định của \gls{dfa}
    và quyết định của
    Teacher trên tập kiểm định - dùng để so sánh tương đối giữa các
    cấu hình $(\theta,
    K)$.
  \item \textbf{\gls{fpr}}: Tỷ lệ gắn attack trên các window bình thường -
    nhãn normal trong
    \gls{lid-ds} không bị nhiễm và có thể sử dụng trực tiếp.
\end{itemize}

\subsection{Xuất cấu hình DFA}
\label{sec:dfa_export}

\gls{dfa} hoàn chỉnh sau bước giải quyết tính không xác định được xuất sang tệp
\texttt{dfa\_config.json}, chứa toàn bộ thông tin cần thiết để nạp
vào \gls{ebpf} Maps ở Giai
đoạn 4. Bảng~\ref{tab:bpf_maps} mô tả cấu trúc ánh xạ giữa các thành
phần \gls{dfa} và các
\gls{bpf} Map tương ứng trong kernel.
...
\begin{table}[H]
  ...
  \begin{tabular}{|
    ...
    \hline
    \textbf{\gls{bpf} Map} & \textbf{Khóa (Key)} & \textbf{Giá trị
    (Value)} \\ \hline
...
M_{\text{syscall}}$ trong bộ nhớ \gls{ebpf} thay vì sử dụng cấu trúc Hash
...
kế mảng hai chiều này cho phép chương trình \gls{ebpf} tra cứu trạng thái
...
Sau khi đánh giá lưới các cấu hình \gls{dfa}, hệ thống chọn cấu hình cuối
cùng theo quy trình trong Thuật toán~\ref{alg:export_dfa}: tiêu chí
chính là
\gls{fpr} nhỏ nhất trong nhóm đạt ngưỡng phát hiện cấp bản ghi tối thiểu
...
thái bắt đầu, bảng
chuyển trạng thái, ánh xạ \gls{syscall}-to-token, lớp trạng thái và
...
hợp lệ cho cặp
\texttt{(state, token)} hiện tại. Cách tách này giữ cho \gls{dfa} là
...
Dựa trên nền tảng lý thuyết về \gls{ebpf} và \gls{bpf} Maps đã trình bày trong
§\ref{sec:ebpf},
chương trình \gls{ebpf} thực hiện các bước sau trên mỗi sự kiện \gls{syscall}:
...
Chi phí xử lý chỉ gồm hai phép tra cứu \gls{bpf} Map $O(1)$ trên mỗi
\gls{syscall} - không cần
...
\begin{table}[H]
  ...
  \resizebox{\textwidth}{!}{
    \begin{tabular}{|l|p{5cm}|p{5.5cm}|}
      ...
      trong \gls{bpf} map. \\ \hline
      \textbf{Nghi vấn} & Trạng thái đích có \texttt{state\_class =
      1}, hoặc token \gls{syscall}
      không có trong bảng chữ cái ($\notin \Sigma$) &
      Thu thập cửa sổ \gls{syscall} và chuyển tới Rust Agent để Transformer
      đánh giá lại.
...
\gls{dfa} được cập nhật để chấp nhận hành vi mới đó. Cơ chế này cho phép hệ
thống hoạt động
...
Một lớp tấn công né tránh phổ biến đối với các \gls{hids} dựa trên
chuỗi \gls{syscall} là
\textbf{mimicry attacks}
\cite{forrest1996sense,grimmer2022improving,wagner2002mimicry}: kẻ
tấn công chèn thêm các \gls{syscall}
trông có vẻ lành tính (\gls{syscall} padding) vào chuỗi khai thác để kéo
giãn tín hiệu bất
thường qua nhiều cửa sổ, giảm điểm bất thường của từng window riêng
lẻ xuống dưới ngưỡng
phát hiện.

Với cấu trúc \gls{dfa} whitelist, lớp tấn công này bị vô hiệu hóa một cách
tự nhiên: \gls{dfa} không
chỉ kiểm tra sự xuất hiện của từng token đơn lẻ mà kiểm tra toàn bộ
\textbf{lộ trình
chuyển trạng thái}. Khi chuỗi khai thác được độn thêm \gls{syscall} bình
thường, con trỏ
trạng thái \gls{dfa} phải đi theo một lộ trình mà các bước chuyển của nó
không có trong tập
huấn luyện. \Gls{dfa} sẽ gặp bước chuyển không xác định và chuyển sang
Attack State, bất
kể độ dài hay bản chất của các \gls{syscall} đệm. Tính chất này là hệ quả
trực tiếp từ cấu
trúc whitelist của \gls{dfa}, không phải là một quy tắc riêng biệt
cần lập trình thêm.

Giới hạn cần lưu ý là các cuộc tấn công được thiết kế đặc biệt để bảo
toàn toàn bộ lộ
trình \gls{dfa} - tức là kẻ tấn công có tri thức về cấu trúc \gls{dfa} và chèn
padding theo cách
...
Hệ thống hỗ trợ cập nhật \gls{dfa} không cần tải lại kernel thông qua cơ
chế cập nhật nguyên
tử của \gls{bpf} Maps như đã mô tả trong §\ref{sec:ebpf}. Khi mô hình
...
\begin{enumerate}
  ...
  \item \gls{dfa} được trích xuất lại (Giai đoạn 3) hoặc cập nhật tăng dần
    trên tập con mới.
  \item Các mục nhập \texttt{transition\_table} mới được ghi nguyên
    tử vào \gls{bpf} Map
    từ user space.
  \item Con trỏ trạng thái của từng \gls{tid} tiếp tục từ vị trí hiện tại
    mà không gián đoạn
    dịch vụ.
\end{enumerate}
