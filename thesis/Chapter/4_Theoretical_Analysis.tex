...
Một trong những ràng buộc kỹ thuật lớn nhất khi triển khai các giải pháp an ninh trong nhân Linux thông qua eBPF là giới hạn về dung lượng bộ nhớ. Nhân hệ điều hành quản lý chặt chẽ tài nguyên bộ nhớ được cấp phát cho các chương trình eBPF nhằm ngăn ngừa các cuộc tấn công từ chối dịch vụ (DoS) do cạn kiệt tài nguyên hệ thống. Do đó, việc phân tích kích thước của DFA và dung lượng bộ nhớ cần thiết để lưu trữ bảng chuyển trạng thái trên cấu trúc BPF Maps là cực kỳ quan trọng.

Bảng chuyển trạng thái của một DFA với tập trạng thái $Q$ (kích thước $|Q| = K$) và bảng chữ cái đầu vào $\Sigma$ (kích thước $|\Sigma|$) được mô tả dưới dạng một hàm số $\delta: Q \times \Sigma \to Q$. Trong trường hợp xấu nhất, số lượng mục nhập (entries) tối đa cần lưu trữ trong bảng chuyển trạng thái là:
...
Xét cấu hình thực nghiệm của hệ thống với kích thước bộ từ vựng $|\Sigma| = 102$ và số lượng trạng thái DFA tối đa $K = 512$, số lượng mục nhập tối đa trong bảng chuyển băm là:

\begin{equation}
  \label{eq:max_entries_ref}
  N_{\text{max}} = 512 \times 102 = 52.224 \text{ mục nhập}
\end{equation}

Để lưu trữ bảng chuyển này trong một BPF Map thuộc kiểu \texttt{BPF\_MAP\_TYPE\_HASH}, khóa (key) của bản đồ cần lưu trữ cặp trạng thái hiện tại và token đầu vào \texttt{(current\_state\_id, token\_id)} có kích thước 8 bytes (sử dụng kiểu \texttt{u32} cho mỗi trường). Giá trị (value) của bản đồ lưu trữ trạng thái tiếp theo \texttt{next\_state\_id} có kích thước 4 bytes (\texttt{u32}). Với mỗi mục nhập tiêu tốn 12 bytes bộ nhớ cho dữ liệu thô, tổng dung lượng bộ nhớ cần thiết để lưu trữ toàn bộ bảng chuyển trong trường hợp xấu nhất ước tính khoảng:

\begin{equation}
  \label{eq:memory_footprint_ref}
  52.224 \times 12 \text{ bytes} \approx 626,7 \text{ KB}
\end{equation}

Mức tiêu thụ bộ nhớ khoảng 626,7 KB này cực kỳ nhỏ và nằm hoàn toàn trong giới hạn cho phép của nhân Linux. Ngay cả khi xét một cấu hình lý thuyết lớn hơn với $K = 500$ và kích thước bộ từ vựng $|\Sigma| = 1000$, số lượng mục nhập tối đa là $500.000$ mục nhập cũng chỉ tiêu tốn khoảng 6 MB dữ liệu thô:

\begin{equation}
  \label{eq:memory_footprint_large}
  500.000 \times 12 \text{ bytes} \approx 6 \text{ MB}
\end{equation}

Các phiên bản nhân hiện đại hỗ trợ cấu hình tham số \texttt{max\_entries} của BPF Maps lên tới hàng triệu mục nhập mà không gặp trở ngại về mặt hiệu năng hay phân bổ tài nguyên.

Tuy nhiên, trong thực tế vận hành, bảng chuyển trạng thái DFA có tính chất thưa thớt (sparsity) cực kỳ cao. Phân phối các lời gọi hệ thống của một chương trình máy chủ thông thường chỉ tập trung vào một tập con rất nhỏ của $\Sigma$, và các chuỗi chuyển dịch trạng thái hợp lệ chỉ chiếm một phần rất nhỏ trong không gian tích bộ đôi $Q \times \Sigma$. Các cặp trạng thái và token không xuất hiện trong dữ liệu huấn luyện bình thường sẽ không được điền vào BPF Map. Khi chương trình eBPF tra cứu một cặp không tồn tại trong bản đồ băm, cơ chế mặc định sẽ coi đó là một bước chuyển sang trạng thái từ chối (rejecting state) hoặc kích hoạt cảnh báo bất thường.

Do đó, số lượng mục nhập thực tế cần lưu trữ trong BPF Map nhỏ hơn rất nhiều so với giới hạn lý thuyết $K \times |\Sigma|$, giúp tiết kiệm đáng kể dung lượng bộ nhớ và tối ưu hóa tốc độ tra cứu của bảng băm trong nhân hệ điều hành.
...
\textbf{Kế hoạch xác thực thực nghiệm:} Để kiểm chứng các lập luận trên, hệ thống sẽ thực hiện đo đạc thực nghiệm tỷ lệ xung đột trên bộ dữ liệu LID-DS-2021 với các kịch bản cấu hình siêu tham số:
\begin{itemize}
  \item Số lượng trạng thái DFA: $K \in \{64, 128, 256, 512\}$
  \item Ngưỡng cắt tỉa thống kê: $\theta \in \{0,95, 0,99, 0,999\}$
\end{itemize}
Các kết quả đo đạc sẽ được báo cáo thông qua hai chỉ số chất lượng chính: độ trung thực (fidelity) của DFA so với Teacher và tỷ lệ báo động giả (False Positive Rate - FPR) tại các cấu hình siêu tham số tương ứng, làm cơ sở lựa chọn cấu hình tối ưu cho hệ thống thực tế.
...
\textbf{Khẳng định:} Việc chèn các syscall đệm độc lập với số lượng $k$ sẽ làm cho con trỏ trạng thái hiện tại của DFA trôi dần vào các trạng thái biên (edge states) hoặc trạng thái từ chối (rejecting states) có tần suất xuất hiện thấp trong dữ liệu bình thường.
...
\begin{enumerate}
  \item Trở thành một trạng thái từ chối (rejecting state) do không tồn tại bước chuyển đi hợp lệ cho syscall tiếp theo trong bảng chuyển trạng thái DFA.
  \item Trở thành một trạng thái biên (edge state) có xác suất chuyển tiếp thấp. Khi kẻ tấn công tiếp tục thực thi các syscall khai thác tiếp theo $e_{i+1}$, bước chuyển dịch từ $q'$ qua $e_{i+1}$ sẽ bị coi là bất thường và kích hoạt cảnh báo.
\end{enumerate}
