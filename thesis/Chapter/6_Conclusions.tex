...
thường dựa trên \gls{syscall} theo hướng lai giữa học sâu và thực thi nhẹ trong nhân hệ điều
...
đường đi \gls{syscall} của kernel, trong khi các hệ thống dựa trên luật tĩnh như Falco hay
...
thường, sau đó chưng cất kết quả học được thành một \gls{dfa} Student đủ nhỏ để nạp vào \gls{bpf} map.
...
huấn luyện Transformer Teacher (Giai đoạn 2), đến chưng cất \gls{dfa} Student qua K-Means (Giai
...
\textbf{Transformer Teacher trên \gls{lid-ds}-2021.} Mô hình Decoder-only Transformer với
...
ổn định qua 7 epoch. Điểm last-token \gls{nll} đạt \gls{auroc} = 0.8503 và \gls{pr-auc} = 0.7093 trên gần
...
\textbf{Chưng cất \gls{dfa} Student.} Cấu hình S3 Majority Voting tại $K = 64$ tạo ra bảng
chuyển trạng thái với 64 states và 1,973 transitions, đạt tỷ lệ phát hiện cấp độ bản ghi
$DR_{\text{rec}} = 90.85\%$, tỷ lệ báo động giả \gls{fpr} = 1.41\% và fidelity 98.12\% so với
Teacher. Kích thước bảng chuyển dưới dạng mảng trực tiếp $64 \times 816 \times 4$ bytes
xấp xỉ 204 KB, hoàn toàn nằm trong giới hạn \gls{bpf} map của các kernel Linux hiện đại.
...
mỗi sự kiện \gls{syscall} tại runtime, phân tích giới hạn bộ nhớ \gls{bpf} map, khung đánh giá fidelity
và đặc tính kháng mimicry attacks của \gls{dfa} whitelist. Khung đánh giá này cũng làm rõ rằng
nhiễu nhãn cấu trúc trong \gls{lid-ds}-2021, nơi phần lớn tail sau khai thác vẫn mang hành vi
bình thường, là lý do tại sao \gls{tpr} cấp độ window thấp không mâu thuẫn với $DR_{\text{rec}}$
...
cluster hơn chỉ làm tăng non-determinism và \gls{fpr} mà không cải thiện
...
Thứ hai, phân tích \gls{mitre-attack} mới dừng lại ở mức khảo sát khả năng ánh xạ. Việc gắn
nhãn các trạng thái attack trong \gls{dfa} với các kỹ thuật tấn công cụ thể như leo thang đặc
quyền hay chiếm quyền điều khiển tiến trình đòi hỏi phân tích sâu hơn về các recording
tấn công trong \gls{lid-ds}.

Thứ ba, mô hình Teacher trong luận văn chỉ sử dụng tên \gls{syscall} như đặc trưng đầu vào,
chưa khai thác tham số \gls{syscall} (như đường dẫn tệp trong \texttt{open}, địa chỉ trong
\texttt{connect}) hay siêu dữ liệu tiến trình. Thiếu thông tin này làm giảm khả năng phân
biệt các hành vi có cùng chuỗi \gls{syscall} nhưng khác nhau về tham số.
...
\textbf{Mở rộng biểu diễn đầu vào bằng tham số \gls{syscall} và siêu dữ liệu tiến trình.} Giới
hạn lớn nhất của cách token hóa hiện tại là chỉ dùng tên \gls{syscall}. Một hướng cải thiện
khả thi là xây dựng từ vựng tổng hợp từ cặp (\gls{syscall}, nhóm tham số), ví dụ phân biệt
...
làm tăng kích thước từ vựng $|\Sigma|$ và có thể làm tăng không gian \gls{dfa}, nhưng có thể
cải thiện đáng kể khả năng phân biệt hành vi khai thác với hành vi bình thường có cùng
chuỗi \gls{syscall}.

\textbf{Đánh giá trên bộ dữ liệu đa dạng và mở rộng ánh xạ \gls{mitre-attack}.} \gls{lid-ds}-2021
là bộ dữ liệu đánh giá duy nhất trong nghiên cứu này nhưng được thu thập trong môi trường container hóa tương đối
đồng nhất. Để đánh giá khả năng tổng quát hóa của pipeline, các thực nghiệm tiếp theo
nên bao gồm các bộ dữ liệu thu thập từ các hệ thống thực tế khác, cũng như các kịch bản
fileless attacks và memory exploitation không để lại dấu vết trên đĩa. Song song với đó,
việc xây dựng ánh xạ có hệ thống giữa các trạng thái \gls{dfa} bị gắn attack với các kỹ thuật
tấn công trong ma trận \gls{mitre-attack} sẽ giúp các analyst có thêm ngữ cảnh diễn giải khi
